...
chown_usage_exit(int status)
{
    ...
    {
        printf("Usage: %s  USERS[:GROUP]  FILE1 FILE2 ...\n", program_name);
...
main(int argc, char *argv[])
{
                           ...
                           "hH", long_options, NULL)) != -1)
    {
        switch(c)
        {
            case 'h':
            case 'H':
                chown_usage_exit(EXIT_SUCESS);
                break;
            default:
                chown_usage_exit(EXIT_FAILURE);
                break;
        }
    }
    ...
    while(optind < argc)
    {
        exit_status |= _chusr2grp(argv[optind++], user);
    }
